...
<Page title="Catalogs" description="Explore related data portals from municipal, national, and private sectors">
    <Section>
        <Card>
            <Searchbar --bg="transparent" --border="none" placeholder="Search Datasources by name or by type"/>
        </Card>
    </Section>
    <Section>
        <SectionHeader title="Results ({data.catalogs.length})">
            {#snippet suffix()}
                <Button variant="primary" onclick={()=> addCatalogModal.show = true}>
